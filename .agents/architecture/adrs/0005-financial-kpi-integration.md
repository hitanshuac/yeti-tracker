...
## Decision
1. **Financial Monetization**: We integrated the EPA's Social Cost of Carbon metric ($190/ton) using the `useeio_financials.csv` to translate abstract kg into a frightening "Carbon Tax Debt ($)". This serves as the primary actionable KPI for the user.
2. **Dataset Creation**: We created `data/carbon_factors.csv`—a robust, verifiable dataset containing the exact CO2 emissions factors and SCC multipliers for various activities (transport, diet, energy).
