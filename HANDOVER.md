...
3. **Verification Gate & Scientific Transparency**: These values populate explicit UI sliders (now supporting granular transport like two-wheelers and auto-rickshaws). The UI renders a `carbon_factors.csv` table exposing CSTEP and CEA emission agencies to eliminate hallucination risk.
...
5. **Statistical Anomaly Detection**: The DuckDB engine calculates the 90th percentile (`PERCENTILE_CONT(0.9)`) across the user's history to flag significant statistical behavioral variations.
